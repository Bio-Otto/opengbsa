...
FILE="7KHZ" #<<<<<<<<<<<<<<<<<<<<<<<<< PUT THE PDB NAME HERE (without the extension)
...
NT=10 #Number of core.
WATER=tip3p #Water type
NUMBEROFREPLICAS=2 #Number of replica
FF=charmm36-jul2022 #Force field amber99sb-ildn
...
#THOSE COMMANDS 
GPU0="-gpu_id 1 -ntomp 2 -nb gpu -bonded gpu -pme gpu -pmefft gpu -pin on -pinstride 0 -nstlist 100 -pinoffset 49" 
GPU1="-gpu_id 1 -ntomp 2 -nb gpu -bonded gpu -pme gpu -pmefft gpu -pin on -pinstride 0 -nstlist 100 -pinoffset 0"
...
EOF
    ...
    acpype -i ligand.mol2 -a gaff2
    mkdir ligand
    mkdir -p ligand #mv ligand* ligand/
    mv ligand.mol2 ligand.acpype ligand/
    mkdir receptor
...
echo "SOL" | $GMX genion -s ions.tpr -o ${PDB}_solv_ions.gro -p topol.top -pname NA -nname CL -neutral -conc 0.15
...
EOF
    ...
    sed -i 's/Protein Non-Protein/Protein_Ligand Water_Ions/g' mdp/nvt_310.mdp
...
for ((i=0; i<$NUMBEROFREPLICAS; i++))
	...
	## temperature 300
	#######################
	$GMX grompp -f mdp/nvt_310.mdp -c results/mini/em.gro -r results/mini/em.gro  -p topol.top -o nvt_310.tpr -maxwarn 2 $INDEX
	$MPI $MDRUN_GPU -deffnm nvt_310 -v 
	#temperature_graph
	...
	#Temparture graph
	echo "16 0" | $GMX energy -f results/nvt/nvt_310.edr -o graph/temperature_nvt_310.xvg

	#######################
	## Pression
	#######################
	$GMX grompp -f mdp/npt.mdp -c results/nvt/nvt_310.gro -r results/nvt/nvt_310.gro -t results/nvt/nvt_310.cpt -p topol.top -o npt_ab.tpr -maxwarn 2 $INDEX
	...
	# Ligand RMSD calculation (the residue named as LIG)
	echo "r LIG\nq" | $GMX make_ndx -f "results/prod/md_"$PDB"_prod.tpr" -o index.ndx
	echo "LIGAND LIGAND" | $GMX rms -s "results/prod/md_"$PDB"_prod.tpr" -f "results/prod/md_"$PDB"_clean_nowat.xtc" -o "graph/prod_"$PDB"_rmsd_ligand.xvg" -tu ns -n index.ndx
	...
	echo "Protein" | $GMX rmsf -s "results/prod/tpr_nowat.tpr" -f "results/prod/md_"$PDB"_clean_nowat.xtc" -o "graph/prod_"$PDB"_rmsf_ref_allatom.xvg" -res
ls
	#####
	...
	then
		...
		if [ -z "$DSSP" ]
			then
				...
				#echo "1" |  $GMX dssp -s "results/prod/tpr_nowat.tpr" -f "results/prod/md_"$PDB"_clean_nowat.xtc" -o "graph/prod_"$PDB"_ss.dat" -tu ns -dt 0.05
				#$GMX xpm2ps -f "graph/prod_"$PDB"_ss.dat" -o "graph/prod_"$PDB"_ss.ps" -by 10 -bx 3
				echo "1" |  $GMX dssp -s "results/prod/tpr_nowat.tpr" -f "results/prod/md_"$PDB"_clean_nowat.xtc" -o "graph/prod_"$PDB"_ss.dat" -tu ns -dt 0.05
				python ../plot_ss.py "graph/prod_"$PDB"_ss.dat" "graph/prod_"$PDB"_ss.png"
			fi
	else
		#echo "Protein" | $GMX dssp -s "results/prod/tpr_nowat.tpr" -f "results/prod/md_"$PDB"_clean_nowat.xtc" -o "graph/prod_"$PDB"_ss.dat" -tu ns -dt 0.05
		#$GMX xpm2ps -f "graph/prod_"$PDB"_ss.dat" -o "graph/prod_"$PDB"_ss.ps" -by 10 -bx 3
		echo "Protein" | $GMX dssp -s "results/prod/tpr_nowat.tpr" -f "results/prod/md_"$PDB"_clean_nowat.xtc" -o "graph/prod_"$PDB"_ss.dat" -tu ns -dt 0.05
		python ../plot_ss.py "graph/prod_"$PDB"_ss.dat" "graph/prod_"$PDB"_ss.png"
	fi
