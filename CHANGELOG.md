...
- **CI environment solve was taking 30+ minutes and timing out**, on both the classic `conda` solver and `mamba install`. Bisected locally by timing `micromamba create` with the dependency set added incrementally: `h5py`, combined with `mdanalysis`'s own dependency tree, pushed the solver into a multi-minute-to-unbounded search even though `h5py` isn't a real dependency of this project (it's commented out in `pyproject.toml` and only used as an optional soft-import in `mmgbsa/utils.py`'s capability check). Removed it from the CI dependency list and from `conda-recipe/meta.yaml`'s `run:` requirements; switched the workflow to `mamba-org/setup-micromamba` (the real micromamba resolver, materially faster than conda's bundled "mamba" command) and dropped the `defaults` channel (routes through Anaconda's rate-limited commercial repo, not needed since every dependency comes from conda-forge). Full environment now solves and installs in under 2 minutes.
